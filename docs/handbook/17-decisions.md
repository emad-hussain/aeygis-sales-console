...
| **Console MFA mode** | **`OPTIONAL`**, TOTP only | Chosen by the user 2026-09-16, ahead of the console being hosted publicly. OPTIONAL lets staff enrol at their own pace; REQUIRED would lock out every account on its next sign-in, including the ones that exist today. Tightening later is a one-word change. TOTP over SMS: no phone numbers to hold, no SNS role, no per-message cost, works abroad, and it is the stronger factor |
| **Where enrolment lives** | **A `2-factor` row in the account panel**, beside role and groups | With OPTIONAL, Cognito never prompts anyone: the Authenticator only forces TOTP setup on a REQUIRED pool. Opting in happens after sign-in through `setUpTOTP()` + `updateMFAPreference()`, which need a screen to be called from. Without one, "MFA enabled" is true of the pool and false of every account. Verified against the installed `@aws-amplify/ui` core: it handles the three sign-in *challenge* steps, but no post-sign-in enrolment |
